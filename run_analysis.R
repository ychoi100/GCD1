...
#
df_labels <- read.table ( "activity_labels.txt" , sep = "" , header = FALSE )
...
#write.table (df_combined , file = "homework1.txt" , row.names = FALSE , col.names = TRUE )
...
for ( x in unique ( df_subject_test$subject ) )
{
  for ( y in unique ( df_labels$V2 ) )
  {
